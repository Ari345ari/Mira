...
import {
  ...
  Bell, X, CheckCheck, XCircle, Building2, Loader2, Folder,
} from 'lucide-react'
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Notification panel — rendered outside aside to avoid overflow:hidden clipping */}
      {notifOpen && (
        <div
          ref={notifRef}
          style={{
            position: 'fixed',
            left: collapsed ? 56 : 216,
            top: 0, bottom: 0,
            width: 320,
            background: '#14121f',
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <button onClick={() => setNotifOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.35)', padding: 4, display: 'flex', borderRadius: 6 }}
            ...
            {invites?.map((inv: any) => (
              <div key={inv.id} style={{ padding: '14px 18px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', margin: '2px 0 0' }}>
                      {inv.workspace?.member_count ?? 0} member{inv.workspace?.member_count !== 1 ? 's' : ''}
                    </p>
                  </div>
                ...
                <div style={{ display: 'flex', gap: 7 }}>
                  <button
                    onClick={() => handleRespond(inv.id, 'accept', inv.workspace?.name ?? 'workspace')}
                  ...
                  <button
                    onClick={() => handleRespond(inv.id, 'decline', inv.workspace?.name ?? 'workspace')}
                    disabled={responding === inv.id}
          ...
          {/* Footer — link to workspaces */}
          <div style={{ padding: '10px 18px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <Link
              href="/workspaces"
              onClick={() => setNotifOpen(false)}
              style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#818cf8', textDecoration: 'none' }}
            >
